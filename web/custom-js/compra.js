...
function agergar_detalle(idproducto,producto,datos,precio_compra,cantidad,exento,perecedero){
   ...
   //          if(exento == 0)
   //          {
   //            agergar_detalle(idproducto,producto,datos,precio_compra,cantidad,0.00,perecedero);
   //            validar_siexistefilas();
   //          } else if (exento == 1){
   //            agergar_detalle(idproducto,producto,datos,precio_compra,cantidad,precio_compra,perecedero);
   //            validar_siexistefilas();
   //          }
  ...
  //Autocomplete Optener productos

    $( "#buscar_producto" ).autocomplete({
      source: function( request, response ) {
        $.ajax( {
          url: "web/ajax/autocomplete_producto.php",
          dataType: "json",
          data: {
            term: request.term
          },
      ...
      return $( "<li>" )
        .append( "<div>" + item.label + "<br> <small>" + item.datos + "</small></div>" )
        .appendTo( ul );
    };
  ...
  var fecha_vence = "";
    ...
    $("#tbldetalle tbody tr").each(function (index)
        ...
        if(total > 0){
            ...
            success: function(data){
              ...
              if(data=="Validado"){
                ...
                success: function(){
                  ...
                  Swal.fire("Ups! Ocurrio un error","Algo salio mal al procesar tu peticion","error");
                        $(dark1).unblock();
                        $(dark2).unblock();
                  }
                ...
                Swal.fire('Lo sentimos, ya existe una compra con este comprobante y fecha!', "Intentalo nuevamente", "warning");
                ...
                Swal.fire('Lo sentimos, no pudimos registrar tu informacion!', "Intentalo nuevamente", "error");
                $(dark1).unblock();
                $(dark2).unblock();
              }
            },error: function() {
           ...
           Swal.fire("Imposible","No se puede registrar una compra con valor 0.00","warning");
           $(dark1).unblock();
           $(dark2).unblock();
        }
